...
public class PlayerSkill : MonoBehaviour
{
    ...
    {   
        ...
        // Detect interactable objects using a raycast  
        ...
        if (Physics.Raycast(ray, out RaycastHit raycastHit, distanceToGrab))
        {
            ...
            {
                RemoveOutline(previousObject); // Clear outline if no interactable object is detected
                previousObject = null; 
            }
        }
        ...
        if (Input.GetMouseButtonDown(0)) Attack(); // Trigger attack logic
        if (Input.GetKeyDown(KeyCode.E)) Skill(); // Trigger skill logic
    ...
    private void Attack()
    {   
        if(attackTimer < attackRate)return; // Ensure the attack cooldown is complete
        PlayerToolController.PlayerTool currentTool = _playerToolController.GetPlayerToolType();

        // Trigger specific attack animations based on the current tool
        switch (currentTool)
        {
    ...
    {   
        ...
        // Trigger specific skill animations based on the tool
        PlayerToolController.PlayerTool currentTool = _playerToolController.GetPlayerToolType();
        switch (currentTool)
        {
            case PlayerToolController.PlayerTool.Hammer:
